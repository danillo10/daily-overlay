...
function render({ running, elapsed, lines, linesPt, partial, translation }) {
  ...
  for (let i = 0; i < originals.length; i += 1) {
    rows.push({ en: originals[i], pt: translated[i] || "" });
  }
  const last = originals.at(-1) || "";
  if (partial && partial !== last) {
    rows.push({ en: partial, pt: translation || "", live: true });
